...
class LokiUpdateController:
    ...
    def __init__(self, options: dict[str, str]):
        # Save arguments
        self.emmc_base_path = str(options.get("emmc_base_path", "/mnt/emmc/"))
        self.sd_base_path = str(options.get("sd_base_path", "/mnt/sd/"))
        self.backup_base_path = str(
            options.get("backup_base_path", "/mnt/emmc/backup/")
        )
        self.allow_reboot = eval(options.get("allow_reboot"))
        self.allow_only_emmc_upload = eval(options.get("allow_only_emmc_upload"))
        self.allow_images_from_repo = eval(options.get("allow_images_from_repo", False))
        self.available_repos = json.loads(options.get("available_repos", "[{}]"))
        ...
        self.installed_images_tree = ParameterTree(
            {
                "emmc": {
                    ...
                    "refresh": (
                        self.get_refresh_emmc_image_info,
                        self.set_refresh_emmc_image_info,
                    ),
                ...
                "sd": {
                    ...
                    "refresh": (
                        self.get_refresh_sd_image_info,
                        self.set_refresh_sd_image_info,
                    ),
                },
                "backup": {
                    "info": (lambda: self.backup_installed_image, None),
                    "refresh": (
                        self.get_refresh_backup_image_info,
                        self.set_refresh_backup_image_info,
                    ),
                },
                "flash": {
                    "info": {
                    ...
                    "refresh": (
                        self.get_refresh_flash_image_info,
                        self.set_refresh_flash_image_info,
                    ),
                ...
                "runtime": {
                    ...
                    "refresh": (
                        self.get_refresh_runtime_image_info,
                        self.set_refresh_runtime_image_info,
                    ),
                },
                "refresh_all_image_info": (
                    self.get_refresh_all_image_info,
                    self.set_refresh_all_image_info,
                ),
            }
        )
        ...
        self.param_tree = ParameterTree(
            {
                ...
                "restrictions": {
                    ...
                    "allow_only_emmc_upload": (
                        lambda: self.allow_only_emmc_upload,
                        None,
                    ),
                    "allow_images_from_repo": (
                        lambda: self.allow_images_from_repo,
                        None,
                    ),
                },
                "github_repos": {
            ...
            try:
                inflight = [
                    ...
                    for x in subprocess.run(
                        ["cat", f"/sys/block/{mmc_blockdev}/inflight"],
                        capture_output=True,
                        text=True,
                        check=True,
                    ).stdout.split(" ")
                ...
                if inflight_write:
                    logging.debug(
                        "Block device {} is still syncing: {}".format(
                            mmc_blockdev, inflight_write
                        )
                    )
                    return False
            except subprocess.CalledProcessError as e:
                logging.error(
                    "Error reading mmc sync state for {}: {}".format(mmc_blockdev, e)
                )
    ...
    def get_installed_image(self, device):
        if device == "runtime":
            (
                ...
                error_occurred,
                error_message,
            ) = self.get_runtime_image_metadata()
        else:
            (
                name,
                app_version,
                ...
                error_occurred,
                error_message,
            ) = self.get_image_metadata_from_dtb(device)
    ...
    def get_image_metadata_from_dtb(self, device):
        ...
        try:
            ...
            metadata_output = subprocess.run(
                ["fdtget", "-t", "s", u_boot_path, "/loki-metadata", "loki-version"],
                capture_output=True,
            )
            ...
            if metadata_output.returncode == 0:
                ...
                platform = subprocess.run(
                    ["fdtget", "-t", "s", u_boot_path, "/loki-metadata", "platform"],
                    capture_output=True,
                    text=True,
                    check=True,
                ).stdout.strip()
            ...
            timestamp_output = subprocess.run(
                ["fdtget", "-t", "i", u_boot_path, "/", "timestamp"],
                capture_output=True,
                text=True,
            )
            ...
            error_occurred = True
            error_message = (
                "Unable to create flattened device tree, .dtb file was not found"
            )
    ...
    def get_flash_image_metadata_from_dtb(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                if metadata_output.returncode == 0:
                    self.flash_app_name = subprocess.run(
                        [
                            "fdtget",
                            "-t",
                            "s",
                    ...
                    self.flash_app_version = subprocess.run(
                        [
                            "fdtget",
                            "-t",
                            "s",
                    ...
                    self.flash_loki_version = subprocess.run(
                        [
                            "fdtget",
                            "-t",
                            "s",
                    ...
                    self.flash_platform = subprocess.run(
                        [
                            "fdtget",
                            "-t",
                            "s",
                    ...
                    timestamp_output = subprocess.run(
                        ["fdtget", "-t", "i", kernel_mtddev, "/", "timestamp"],
                        capture_output=True,
                        text=True,
                        check=True,
                    )
                    ...
                    self.flash_app_name = json_fdt["loki-metadata"]["application-name"][
                        1
                    ]
                    self.flash_app_version = json_fdt["loki-metadata"][
                        "application-version"
                    ][1]
                    self.flash_loki_version = json_fdt["loki-metadata"]["loki-version"][
                        1
                    ]
    ...
    def get_runtime_image_metadata(self):
        ...
        try:
            with open(
                "/sys/firmware/devicetree/base/loki-metadata/application-name", "r"
            ) as file:
                name = file.read()
                file.close()

            with open(
                "/sys/firmware/devicetree/base/loki-metadata/application-version", "r"
            ) as file:
                app_version = file.read()
                file.close()

            with open(
                "/sys/firmware/devicetree/base/loki-metadata/loki-version", "r"
            ) as file:
                loki_version = file.read()
                file.close()

            with open(
                "/sys/firmware/devicetree/base/loki-metadata/platform", "r"
            ) as file:
                platform = file.read()
                file.close()
    ...
    def mtd_label_to_device(self, label):
        try:
            device_list_output = subprocess.run(
                ["lsmtd", "-r"], capture_output=True, text=True, check=True
            )
    ...
    def upload_file(self, files):
        ...
        for file in file_names:
            ...
            checksum = next(
                item for item in self.checksums if item["fileName"] == file
            )["checksum"]
    ...
    def copy_file(self, src, dest):
        ...
        with open(src_path, "rb") as src_file:
            with open(dest_path, "wb") as dest_file:
                self.copy_file_object(
                    src_file, dest_file, buffer_size=buffer_size, total=size
                )
            ...
            try:
                if file_extension == "ub":
                    process = subprocess.Popen(
                        ["flashcp", "-v", src_path, self.mtd_label_to_device("kernel")],
                    ...
                    process = subprocess.Popen(
                        ["flashcp", "-v", src_path, self.mtd_label_to_device("boot")],
                        stdout=subprocess.PIPE,
                        bufsize=1,
                        universal_newlines=True,
                    )
                    ...
                    process = subprocess.Popen(
                        [
                            ...
                            "-v",
                            src_path,
                            self.mtd_label_to_device("bootscr"),
                        ],
                        stdout=subprocess.PIPE,
                        bufsize=1,
                        universal_newlines=True,
    ...
    def get_release_tags_from_repo(self, owner, repo):
        release_response = requests.get(
            f"{GITHUB_REPO_API_URL}/{owner}/{repo}/releases"
        )
    ...
    def set_release_to_retrieve(self, release):
        ...
        tag = release.get("tag")
        owner = next(
            (item for item in self.available_repos if item.get("name") == repo)
        ).get("owner")
    ...
    def download_release_assets(self, owner, repo, tag):
        ...
        tag_response = requests.get(
            f"{GITHUB_REPO_API_URL}/{owner}/{repo}/releases/tags/{tag}"
        )
